...
export function stripArabic(text) {
  ...
  let result = t.replace(/[٠١٢٣٤٥٦٧٨٩]/g, d => indicMap[d])
  ...
  for (const [word, num] of Object.entries(ORDINALS).sort(([a], [b]) => b.length - a.length)) {
    result = result.replace(new RegExp(`\\s*${word.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*`, 'gi'), ` ${num} `)
  }

  result = result.replace(/\s*الموسم\s*(\d+)\s*/gi, (_, num) => ` Season ${num} `)
  result = result.replace(/\s*الحلقة\s*(\d+)\s*/gi, (_, num) => ` Episode ${num} `)
  ...
  for (const word of noise) {
    result = result.replace(new RegExp(`\\s*${word.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*`, 'gi'), ' ')
  }
